...
bool Sprite::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, long screenWidth, long screenHeight, const char* spriteFilename, int renderX, int renderY)
{
  bool result = false;
...
bool Sprite::LoadTextures(ID3D11Device* device, ID3D11DeviceContext* deviceContext, const char* filename)
{
  ...
  for (long i = 0; i < m_textureCount; i++)
  {
    long j = 0;
    fin.get(input);
    while (input != '\n')
    ...
    long pos = m_Textures.size();
    m_Textures.emplace_back(make_unique<Texture>());
    result = m_Textures[pos]->Initialize(device, deviceContext, textureFilename);
    if (!result)
    {
      return false;
    }
