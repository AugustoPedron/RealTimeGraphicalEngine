...
bool TextureShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"D:\\ProgettiPersonali\\RealTimeGraphicalEngine\\shaders\\Texture.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"D:\\ProgettiPersonali\\RealTimeGraphicalEngine\\shaders\\Texture.ps");
	if (error != 0)
	{
		return false;
	}
