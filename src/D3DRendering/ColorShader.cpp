...
bool ColorShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFilename, 128, L"D:\\ProgettiPersonali\\RealTimeGraphicalEngine\\shaders\\Color.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"D:\\ProgettiPersonali\\RealTimeGraphicalEngine\\shaders\\Color.ps");
	if (error != 0)
	{
		return false;
	}
...
bool ColorShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix)
{
	HRESULT result;
